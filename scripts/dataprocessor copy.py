...
parser.add_argument("--openaiendpoint", help="URL of the Azure OpenAI service to be used for embeddings and chatbot (must exist already)")
parser.add_argument("--azureopenaiapiversion", help="API version to use with the Azure OpenAI service")
...
# Create an Azure OpenAI service client
import openai
from azure.identity import DefaultAzureCredential
...
openai.api_base = "{args.openaiendpoint}"
openai.api_version = "{args.azureopenaiapiversion}"
...
def create_search_index():
    ...
    fields.append(simple_field)
    simple_field = SearchField(name="contentVector", type=SearchFieldDataType.Collection(SearchFieldDataType.Single),searchable=True, dimensions=1536, vector_search_configuration="my-vector-config")
    fields.append(simple_field)
